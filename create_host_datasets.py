...
TOPO_SCALAR_COLUMNS = ["elev_mean","elev_sd","elev_min","elev_max","slope_mean","slope_sd","northness_mean","eastness_mean","topo_valid_frac"]
...
def parse_args():
    ...
    parser.add_argument("--topo-mode", choices=["none", "chip"], default="none")
...
def extract_topo_for_naip_chip(naip_chip_fp: str, topo_sources: dict, out_fp: str | None, topo_chip_size: int):
    ...
    if elev.size:
        stats.update({"elev_mean": float(np.nanmean(elev)), "elev_sd": float(np.nanstd(elev)), "elev_min": float(np.nanmin(elev)), "elev_max": float(np.nanmax(elev),), "slope_mean": float(np.nanmean(slope)), "slope_sd": float(np.nanstd(slope)), "northness_mean": float(np.nanmean(n)), "eastness_mean": float(np.nanmean(e))})
    else:
