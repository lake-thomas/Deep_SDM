...
# Respect scheduler/conda-provided GDAL/PROJ paths on Linux/HPC.
conda_prefix = os.environ.get("CONDA_PREFIX")
if conda_prefix:
    os.environ.setdefault("GDAL_DATA", os.path.join(conda_prefix, "share", "gdal"))
    os.environ.setdefault("PROJ_LIB", os.path.join(conda_prefix, "share", "proj"))
os.environ["PROJ_NETWORK"] = "OFF"
if os.environ.get("PROJ_LIB"):
    os.environ.setdefault("PROJ_DATA", os.environ["PROJ_LIB"])

# Legacy Windows workstation fallback; never override an existing HPC environment.
conda_env = r"C:\Users\talake2\AppData\Local\anaconda3\envs\naip_ailanthus_env"
if os.name == "nt" and os.path.isdir(conda_env):
...
def list_naip_tiles(
    naip_folder: str | None,
    tile_fp: str | None,
    tile_list_fp: str | None = None,
    ...
    if tile_fp:
        ...
        return [path]
    if tile_list_fp:
        list_path = Path(tile_list_fp)
        if not list_path.exists():
            raise FileNotFoundError(list_path)
        tiles = [Path(line.strip()) for line in list_path.read_text().splitlines() if line.strip()]
        missing = [str(path) for path in tiles if not path.exists()]
        if missing:
            raise FileNotFoundError(f"Missing tile(s) from --tile-list: {missing[:5]}")
        return tiles
    if not naip_folder:
        raise ValueError("Provide --tile-fp, --tile-list, or --naip-folder.")
...
def parse_wgs84_bounds(value: str | None) -> tuple[float, float, float, float] | None:
    """Parse west,south,east,north bounds in EPSG:4326 coordinates."""
    if not value:
        return None
    parts = [float(x.strip()) for x in value.split(",")]
    if len(parts) != 4:
        raise ValueError("Bounds must be west,south,east,north in EPSG:4326.")
    west, south, east, north = parts
    if not west < east or not south < north:
        raise ValueError(f"Invalid bounds order: {value}")
    return west, south, east, north


def read_raster_for_optional_wgs84_bounds(
    ds: rasterio.io.DatasetReader,
    bounds_wgs84: tuple[float, float, float, float] | None,
) -> tuple[np.ndarray, Any]:
    """Read a full raster or clipped EPSG:4326 bounds while preserving transform."""
    if bounds_wgs84 is None:
        return ds.read(1).astype(np.float32), ds.transform

    left, bottom, right, top = transform_bounds(
        "EPSG:4326", ds.crs, *bounds_wgs84, densify_pts=21
    )
    window = from_bounds(left, bottom, right, top, transform=ds.transform)
    window = window.round_offsets().round_lengths()
    arr = ds.read(1, window=window, boundless=True, masked=True).astype(np.float32)
    if np.ma.isMaskedArray(arr):
        arr = arr.filled(np.nan)
    return np.asarray(arr, dtype=np.float32), ds.window_transform(window)
...
def preload_and_normalize_env_rasters(
    ...
    """Preload WorldClim/GHM rasters, optionally clipped, using training stats."""
    ...
    for varname in WC_VARS:
        ...
        with rasterio.open(raster_fp) as ds:
            arr, transform = read_raster_for_optional_wgs84_bounds(ds, bounds_wgs84)
...
def build_tile_topo_scalar_cache(
    naip_src: rasterio.io.DatasetReader,
    rows: list[int],
    cols: list[int],
    topo_sources: dict[str, rasterio.io.DatasetReader],
    ...
    normalize_topography: bool,
) -> dict[tuple[int, int], dict[str, float] | None]:
    """Precompute topo scalar summaries for non-overlapping chips in one tile-level warp."""
    if not rows or not cols:
        return {}
    full_window = Window(
        col_off=cols[0],
        row_off=rows[0],
        width=(cols[-1] - cols[0]) + chip_size,
        height=(rows[-1] - rows[0]) + chip_size,
    )
    bounds = rasterio.windows.bounds(full_window, naip_src.transform)
    # Keep the tile-level warp at the same effective resolution as each chip-level
    # topo extraction, then take overlapping blocks when stride < chip_size.
    scale = topo_chip_size / float(chip_size)
    dst_h = int(round(full_window.height * scale))
    dst_w = int(round(full_window.width * scale))
    row_offsets = [int(round((row - rows[0]) * scale)) for row in rows]
    col_offsets = [int(round((col - cols[0]) * scale)) for col in cols]
    if dst_h <= 0 or dst_w <= 0:
        raise ValueError("Invalid tile-level topo cache dimensions.")
    dst_transform = rasterio.transform.from_bounds(*bounds, width=dst_w, height=dst_h)
    ...
    if ("northness" not in layers or "eastness" not in layers) and topo_sources.get("aspect") is not None:
        aspect = topo_sources["aspect"]
        aspect_arr = np.full((dst_h, dst_w), np.nan, dtype=np.float32)
        reproject(
            source=rasterio.band(aspect, 1),
            destination=aspect_arr,
            src_transform=aspect.transform,
    ...
    raw_stack = np.stack([layers[name].astype(np.float32) for name in TOPO_IMAGE_BANDS], axis=0)
    valid = np.isfinite(raw_stack).all(axis=0)
    model_stack = normalize_topo_stack(raw_stack, topo_norm_stats) if normalize_topography else raw_stack

    cache: dict[tuple[int, int], dict[str, float] | None] = {}
    for row_idx, row in enumerate(rows):
        r0 = row_offsets[row_idx]
        r1 = r0 + topo_chip_size
        for col_idx, col in enumerate(cols):
            c0 = col_offsets[col_idx]
            c1 = c0 + topo_chip_size
            block_valid = valid[r0:r1, c0:c1]
            if float(block_valid.mean()) < topo_min_valid_frac:
                cache[(row, col)] = None
                continue
            stats = summarize_topo_stack(model_stack[:, r0:r1, c0:c1], block_valid)
            stats["topo_normalized"] = float(bool(normalize_topography))
            cache[(row, col)] = stats
    return cache



def vector_from_feature_map(feature_map: dict[str, float], env_vars: list[str]) -> list[float] | None:
    vec: list[float] = []
    for col in env_vars:
...
def sample_preloaded_raster_points(
    lons: np.ndarray,
    lats: np.ndarray,
    raster_info: dict[str, Any],
) -> np.ndarray:
    xs, ys = transform_coords("EPSG:4326", raster_info["crs"], lons.tolist(), lats.tolist())
    rows, cols = rowcol(raster_info["transform"], xs, ys)
...
def build_tile_env_base_cache(
    naip_src: rasterio.io.DatasetReader,
    chip_iter: list[tuple[int, int]],
    chip_size: int,
    wc_rasters: dict[str, dict[str, Any]],
    ghm_raster: dict[str, Any],
    lat_lon_stats: dict[str, Any],
) -> dict[tuple[int, int], dict[str, float]]:
    centers = [naip_src.transform * (col + chip_size / 2.0, row + chip_size / 2.0) for row, col in chip_iter]
    xs = [xy[0] for xy in centers]
    ys = [xy[1] for xy in centers]
    lons_raw, lats_raw = transform_coords(naip_src.crs, "EPSG:4326", xs, ys)
    lons = np.asarray(lons_raw, dtype=np.float64)
    lats = np.asarray(lats_raw, dtype=np.float64)

    values: dict[str, np.ndarray] = {var: sample_preloaded_raster_points(lons, lats, wc_rasters[var]) for var in WC_VARS}
    values["ghm"] = sample_preloaded_raster_points(lons, lats, ghm_raster)
    values["lat_norm"] = ((lats - float(lat_lon_stats["lat"]["mean"])) / float(lat_lon_stats["lat"]["std"])).astype(np.float32)
    values["lon_norm"] = ((lons - float(lat_lon_stats["lon"]["mean"])) / float(lat_lon_stats["lon"]["std"])).astype(np.float32)

    cache: dict[tuple[int, int], dict[str, float]] = {}
    for idx, loc in enumerate(chip_iter):
        cache[loc] = {name: float(vals[idx]) for name, vals in values.items()}
    return cache
...
def run_inference_on_tile(
    ...
    with rasterio.open(tile_fp) as src:
        ...
        rows = list(range(0, height - chip_size + 1, stride))
        cols = list(range(0, width - chip_size + 1, stride))
        chip_iter = [(r, c) for r in rows for c in cols]

        topo_scalar_cache = None
        if fast_topo_scalars and topo_needed_for_env and not req["topo"]:
            topo_scalar_cache = build_tile_topo_scalar_cache(
                naip_src=src,
                rows=rows,
                cols=cols,
        ...
        env_base_cache = None
        if req["env"]:
            env_base_cache = build_tile_env_base_cache(
                naip_src=src,
                chip_iter=chip_iter,
                chip_size=chip_size,
        ...
        for row, col in tqdm(chip_iter, desc=f"{tile_name}", leave=False):
            ...
            if req["image"]:
                ...
                if not naip_window_is_valid(chip, min_valid_frac=min_image_valid_frac):
                    skipped_image += 1
                    continue
                if naip_scale_255:
                    chip = chip / 255.0
            ...
            topo_stats: dict[str, float] | None = None
            if topo_needed:
                if topo_scalar_cache is not None:
                    topo_stats = topo_scalar_cache.get((row, col))
            ...
            if req["env"]:
                if env_base_cache is not None:
                    feature_map = dict(env_base_cache[(row, col)])
                    if topo_stats:
                        feature_map.update(topo_stats)
                    env_vector = vector_from_feature_map(feature_map, env_vars)
                else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tile-list", type=str, default=None, help="Text file with one NAIP GeoTIFF path per line.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional EPSG:4326 west,south,east,north bounds for preloaded WorldClim/GHM rasters. Normalization still uses training stats.",
    )
    parser.add_argument(
        "--env-clip-conus",
        action="store_true",
        help="Clip preloaded WorldClim/GHM rasters to approximate lower-48/CONUS bounds before applying training normalization stats.",
    )
    ...
    parser.add_argument("--fast-topo-scalars", action="store_true", help="Precompute tile-level topo scalar summaries for non-overlapping tabular topo features.")
...
def main() -> None:
    ...
    if req["env"]:
        ...
        env_clip_bounds = parse_wgs84_bounds(args.env_clip_bounds)
        if args.env_clip_conus:
            env_clip_bounds = (-125.0, 24.0, -66.0, 50.0)
        if env_clip_bounds:
            print(f"Preloading and normalizing WorldClim/GHM rasters clipped to EPSG:4326 bounds {env_clip_bounds}...")
        else:
            print("Preloading and normalizing WorldClim/GHM rasters...")
        wc_rasters, ghm_raster = preload_and_normalize_env_rasters(worldclim_dir, ghm_path, wc_stats, bounds_wgs84=env_clip_bounds)
    ...
    tiles = list_naip_tiles(args.naip_folder, args.tile_fp, tile_list_fp=args.tile_list, recursive=args.recursive)
    if not tiles:
    ...
    try:
        for tile_fp in tqdm(tiles, desc="Processing tiles"):
            try:
                ...
                traceback.print_exc()
                summaries.append({"tile": str(tile_fp), "status": "error", "error": str(exc), "traceback": traceback.format_exc()})
    finally:
